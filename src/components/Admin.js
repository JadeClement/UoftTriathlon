...
const Admin = () => {
  ...
  const [selectedUserResults, setSelectedUserResults] = useState([]); // eslint-disable-line no-unused-vars -- used via context
  const [selectedUserResultsLoading, setSelectedUserResultsLoading] = useState(false); // eslint-disable-line no-unused-vars -- used via context
  ...
  // Load orders when on orders route
  const isOnOrdersRoute = location.pathname.endsWith('/orders');
  useEffect(() => {
  ...
  // Load Interval Results users list when on interval-results route or date filter changes
  const isOnIntervalResultsRoute = location.pathname.endsWith('/interval-results');
  useEffect(() => {
    const loadIntervalUsers = async () => {
      if (!isOnIntervalResultsRoute) return;
  ...
  const adminContextValue = {
    ...
    selectedUserResults, selectedUserResultsLoading, intervalDateFilter, setIntervalDateFilter,
