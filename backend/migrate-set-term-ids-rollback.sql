...
--    Use migrate-set-term-ids-rollback-apply.sql to actually apply the rollback
-- 
-- This migration will:
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Reverse Migration: set-term-ids-rollback (DRY-RUN)';
...
BEGIN
  ...
  RAISE NOTICE '[METRICS] Users that would have term_id cleared: %', users_to_rollback;
...
-- Show sample of what would be rolled back
SELECT 
  u.id,
  u.name,
  u.email,
...
BEGIN
  ...
  RAISE NOTICE '[%] Starting ROLLBACK operation (DRY-RUN - will be rolled back)...', rollback_start_time;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '[%] ROLLBACK operation completed (DRY-RUN)', rollback_end_time;
...
BEGIN
  ...
  RAISE NOTICE '========================================';
  RAISE NOTICE '[%] Rollback Summary (DRY-RUN)', migration_end_time;
  RAISE NOTICE '========================================';
END $$;
...
-- 
-- To actually apply this rollback:
--   1. Review all the SELECT output above
--   2. If everything looks correct, change ROLLBACK to COMMIT below
--   3. Or use migrate-set-term-ids-rollback-apply.sql which already has COMMIT
--
