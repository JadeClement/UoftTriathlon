...
-- THIS VERSION ASSUMES cancelled_at WAS STORED IN UTC
-- Use this if you see negative hours_diff values in the other script
-- 
...
-- This version treats cancelled_at as UTC (common for servers)
...
SELECT 
  ...
  fp.workout_date,
  fp.workout_time,
  -- Combine workout date and time as local timestamp in Toronto timezone
  (fp.workout_date::timestamp + fp.workout_time::time) AS workout_datetime,
  -- cancelled_at was stored in UTC, so we need to:
  -- 1. Treat it as UTC (AT TIME ZONE 'UTC' creates a timestamptz)
  -- 2. Convert to Toronto timezone for comparison
  (wc.cancelled_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Toronto') AS cancelled_at_toronto,
  -- Calculate hours between workout and cancellation (both in Toronto timezone)
...
-- Show records with negative hours_diff (shouldn't happen with UTC conversion)
SELECT 
